...
class KubernetesPodWatcher(KubernetesRunner):
    ...
    def _read_from_stream(self, num_bytes: int) -> str:
        ...
        with self._ws_lock:
            if self.process and self.process.closed:
    ...
    @property
    def process_is_finished(self) -> bool:
        if not self.process or not self.process.closed:
            return False
        ...
        with self._ws_lock:
            if TestConfig().tester_obj().teardown_started:
                LOGGER.warning(
                    ...
                    pod_name, "closed" if self.process and self.process.closed else "open")
                self._stop_pod()
                return
            if self.process and self.process.closed:
                LOGGER.warning(
